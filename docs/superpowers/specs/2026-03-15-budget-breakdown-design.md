...
- `status` — `'estimated'` | `'agreed'` | `'paid'`. Tracks pricing lifecycle.
...
2. Implementation: application-level logic in the inquiry response hook (not a database trigger, for flexibility)

## Pricing Entry Points

**Either party can enter/update pricing:**
- **Vendor side:** `RespondInquiryDialog` gains an optional "Your Price" number input and "Notes" text field. Price is saved to the budget item on acceptance.
- **User side:** Budget breakdown view on the event detail page. Click any row to edit price and notes inline. Entering a price on an estimated item moves status to `'agreed'`.

## UI Changes

### 1. New Event Detail Page (`/events/:id`)

Accessed by clicking an EventCard on the dashboard. Contains:
...
- **Event owner:** Full read/write on all budget items for their events
- **Vendor:** Read and update only their own budget items (matched by `vendor_id`). Cannot see other vendors' items.
- **Public:** No access
...
- **Inquiry declined after budget item created:** Budget item stays but is flagged. User can delete or replace
