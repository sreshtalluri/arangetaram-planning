...
BEGIN
  ...
  IF NEW.status = 'accepted' AND (OLD.status IS NULL OR OLD.status != 'accepted') THEN
    -- Only proceed if vendor profile exists
    IF EXISTS (SELECT 1 FROM public.vendor_profiles WHERE id = NEW.vendor_id) THEN
      ...
      -- Create or re-activate budget item (handles re-accept after decline)
      IF EXISTS (SELECT 1 FROM public.event_budget_items WHERE inquiry_id = NEW.id) THEN
        UPDATE public.event_budget_items
        SET status = CASE WHEN NEW.quoted_price IS NOT NULL THEN 'agreed' ELSE 'estimated' END,
            agreed_price = NEW.quoted_price,
            price_notes = NEW.quoted_price_notes
